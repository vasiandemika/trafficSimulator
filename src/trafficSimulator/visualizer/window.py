...
class Window:
    ...
    def draw_segments(self):
        for segment in self.simulation.segments:
            ...
            # dpg.draw_arrow(segment.points[-1], segment.points[-2], thickness=0, size=2, color=(0, 0, 0, 50),
            # parent="Canvas")

    def draw_traffic_lights(self):
        for tl in self.simulation.traffic_lights:
            # Determine the position based on its segment or hardcode for simplicity. # Replace with actual position
            color = (255, 0, 0) if tl.state == 'RED' else (0, 255, 0)
            dpg.draw_circle(tl.position, radius=2 * self.zoom, color=color, thickness=1.76 * self.zoom, parent="Canvas")
